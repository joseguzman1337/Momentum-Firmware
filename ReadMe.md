...
<details>
<summary><strong>✅ v1.0.22 NX Augmented — delivery state</strong></summary>

The `v1.0.22-nx-augmented` updater is built for **Flipper Zero target 7**. Its build date, version, release tag, branch identity, and source commit are generated from the same build metadata so the device and release artifacts identify the exact compilation rather than carrying manually maintained values.
...
- **Flipper Lab application compatibility:** every generated FIM uses the catalog's unique 24-character hexadecimal application and version IDs, normalized icon, and exact `/ext/apps/...` FAP path. Lab can therefore enumerate the complete official-catalog application set without fabricated identities or catalog HTTP 422 responses.
- **Hardened Flipper Lab RPC:** property, device-information, power-information, storage-info, file-stat, timestamp, and storage-read responses now initialize a fresh protobuf envelope and the correct response union for every message. Error cleanup is safe even when a requested file cannot be opened, preventing stale payloads and session corruption during `getInfo()` and application discovery.
- **Reliable RPC transport and screen-stream shutdown:** framebuffer callbacks never wait for the RPC transport; contended frames are dropped as best-effort traffic, leave 768 bytes available for a complete maximum-size manifest-read/protobuf response, and may queue no more than 2 KiB ahead of reliable replies. The full 32 KiB pipe remains available for lossless property, storage, and application-inventory bursts without firmware-side timeouts. Stop first removes the callback, then terminates and joins the worker before acknowledging, so no frame can cross the stop boundary and no canvas callback can target a stopped task.
...
- **Firmware-space guard:** CPU1 size, protected radio/FUS regions, exported API compatibility, and remaining flash headroom are checked during the build. User-facing capabilities stay in FAP/FAL resources where possible instead of silently consuming the protected firmware regions.
- **Regression coverage:** automated tests cover RPC response initialization and union selection, GUI stream shutdown, Marketplace receipts, IDs, icons and FIM paths, incomplete syncs, hash mismatches, missing builds, unsafe paths, invalid identities, and preservation of the last good destination on failure.
- **Reproducible updater bundle:** firmware, radio stack, resources, splash data, updater metadata, SDK, DFU, and full binary are produced from one source revision; reserve approximately **15–30 minutes** for a normal full-device update, with USB reconnection and on-device validation required before declaring success.
...
<img src=".github/assets/badkb-012.png" align="left" width="250px"/>
