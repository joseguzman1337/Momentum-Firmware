...
<details>
<summary><strong>✅ v1.0.17 NX Augmented — verified delivery state</strong></summary>

The `v1.0.17-nx-augmented` updater is built for **Flipper Zero target 7**. Its build date, version, release tag, branch identity, and source commit are generated from the same build metadata so the device and release artifacts identify the exact compilation rather than carrying manually maintained values.
...
- **Reliable screen-stream shutdown:** GUI stream stop acknowledges the request before worker teardown, treats an already-idle stream as a successful stop, and keeps screen frames best-effort so a stalled viewer cannot block device information or navigation requests.
