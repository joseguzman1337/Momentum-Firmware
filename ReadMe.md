...
<details>
<summary><strong>✅ v1.0.23 NX Augmented — delivery state</strong></summary>

The `v1.0.23-nx-augmented` updater is built for **Flipper Zero target 7**. Its build date, version, release tag, branch identity, and source commit are generated from the same build metadata so the device and release artifacts identify the exact compilation rather than carrying manually maintained values.
...
- **Reliable RPC transport and screen-stream shutdown:** framebuffer callbacks never wait for the RPC transport; a frame is admitted only while the USB transmit pipe is completely idle and contended frames are dropped as best-effort traffic. Reliable property, storage, stream-control, and application-inventory replies therefore take priority while retaining the full 32 KiB lossless pipe. Stop first removes the callback, then terminates and joins the worker before acknowledging, so no frame can cross the stop boundary and no canvas callback can target a stopped task.
