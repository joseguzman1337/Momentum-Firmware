...
<details>
<summary><strong>✅ v1.0.18 NX Augmented — delivery state</strong></summary>

The `v1.0.18-nx-augmented` updater is built for **Flipper Zero target 7**. Its build date, version, release tag, branch identity, and source commit are generated from the same build metadata so the device and release artifacts identify the exact compilation rather than carrying manually maintained values.
...
- **Reliable screen-stream shutdown:** framebuffer callbacks never wait for the RPC transport; contended frames are dropped as best-effort traffic. Stop first removes the callback, then terminates and joins the worker before acknowledging, so no frame can cross the stop boundary and no canvas callback can target a stopped task. USB streaming also reserves pipe capacity for control replies.
