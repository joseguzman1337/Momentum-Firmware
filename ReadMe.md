...
<details>
<summary><strong>✅ v1.0.21 NX Augmented — delivery state</strong></summary>

The `v1.0.21-nx-augmented` updater is built for **Flipper Zero target 7**. Its build date, version, release tag, branch identity, and source commit are generated from the same build metadata so the device and release artifacts identify the exact compilation rather than carrying manually maintained values.
...
- **Reliable RPC transport and screen-stream shutdown:** framebuffer callbacks never wait for the RPC transport; contended frames are dropped as best-effort traffic and leave 768 bytes available for a complete maximum-size manifest-read/protobuf response. Reliable USB replies keep their original lossless backpressure behavior without firmware-side timeouts. Stop first removes the callback, then terminates and joins the worker before acknowledging, so no frame can cross the stop boundary and no canvas callback can target a stopped task.
...
- **Reproducible updater bundle:** firmware, radio stack, resources, splash data, updater metadata, SDK, DFU, and full binary are produced from one source revision; a normal full-device update takes approximately **10 minutes**, with USB reconnection and on-device validation required before declaring success.
