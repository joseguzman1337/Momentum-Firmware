...
def test_stop_stream_removes_callback_before_stopping_worker_and_acknowledging():
    ...
    cleanup_start = source.index("void rpc_system_gui_quiesce_context")
    start = source.index("static void rpc_system_gui_stop_screen_stream_process")
    cleanup = source[cleanup_start:start]
    end = source.index("static void\n    rpc_system_gui_send_input_event_request_process", start)
    handler = source[start:end]
    exit_signal = cleanup.index("RpcGuiWorkerFlagExit")
    acknowledgement = handler.index("rpc_send_and_release_empty")
    remove_callback = cleanup.index("gui_remove_framebuffer_callback")
    join = cleanup.index("furi_thread_join")
    assert remove_callback < exit_signal < join
    assert handler.index("rpc_system_gui_quiesce_context") < acknowledgement
...
def test_session_cleanup_removes_callback_before_stopping_worker():
    ...
    assert "rpc_system_gui_quiesce_context(context);" in cleanup
