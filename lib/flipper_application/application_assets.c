...
static bool flipper_application_assets_process_files(
    ...
    for(uint32_t i = 0; i < files_count; i++) {
        ...
        path_concat(full_path, path, file_path);
...
static AssetsSignatureResult flipper_application_assets_process_signature(
    Storage* storage,
    File* file,
    const char* signature_file_path,
    uint8_t** signature_data,
    size_t* signature_data_size) {
    furi_assert(storage);
    furi_assert(file);
    furi_assert(signature_file_path);
    ...
    do {
        ...
        *signature_data =
        ...
        if(!storage_file_open(
               signature_file, signature_file_path, FSAM_READ_WRITE, FSOM_OPEN_EXISTING)) {
...
bool flipper_application_assets_load(File* file, const char* appid, size_t offset, size_t size) {
    ...
    FURI_LOG_D(TAG, "Loading assets for %s", appid);

    FuriString* full_path = furi_string_alloc_printf(STORAGE_APPS_ASSETS_STEM "/%s", appid);
    FuriString* signature_file_path = furi_string_alloc_printf(
        "%s/%s", furi_string_get_cstr(full_path), FLIPPER_APPLICATION_ASSETS_SIGNATURE_FILENAME);
    ...
    do {
        ...
        if(header.dirs_count &&
           !flipper_application_assets_process_dirs(
               storage, file, furi_string_get_cstr(full_path), header.dirs_count)) {
            break;
        }
        ...
        if(header.files_count &&
           !flipper_application_assets_process_files(
               storage, file, furi_string_get_cstr(full_path), header.files_count)) {
            break;
        }
